...
import (
	...
	constoller "github.com/ubombar/live-pod-migration/pkg/controllers/livepodmigration/v1alpha1"
...
func main() {
	...
	lpmClient, err := clientset.NewForConfig(cfg)
	if err != nil {
		klog.Fatalf("Error building example clientset: %s", err.Error())
	}

	kubeInformerFactory := kubeinformers.NewSharedInformerFactory(kubeClient, time.Second*30)
	lpmInformerFactory := informers.NewSharedInformerFactory(lpmClient, time.Second*30)

	controller := constoller.NewController(
		kubeClient,
		lpmClient,
	...
	lpmInformerFactory.Start(stopCh)
